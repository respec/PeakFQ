...
FileCancel:
            ...
            For i = .FixedRows To .Rows - 1
                If IsNumeric(.CellValue(i, 0)) AndAlso IsNumeric(.CellValue(i, 1)) AndAlso
                   ...
                   (IsNumeric(.CellValue(i, 3)) Or (.CellValue(i, 3) IsNot Nothing AndAlso .CellValue(i, 3).ToLower.Contains("inf"))) Then
                    ...
                    If lThresh.SYear <= lStn.EndYear AndAlso lThresh.EYear >= lStn.BegYear Then
                        If lThresh.SYear < lStn.BegYear Then lThresh.SYear = lStn.BegYear
                        If lThresh.EYear > lStn.EndYear Then lThresh.EYear = lStn.EndYear
    ...
    Private Sub UpdateInputGraph(Optional ByVal aSaveToFile As Boolean = False)
        ...
        Dim lThreshFound As Integer
        Dim lThreshIndex As Integer
        lIntervalsPlotted = False
        For Each lQ In lAllObs
            lThreshIndex = 0
            lThreshFound = -1
            For Each vThresh In lStn.Thresholds
                If lQ.Year >= vThresh.SYear AndAlso lQ.Year <= vThresh.EYear Then
                    'current peak is within this threshold range
                ...
                lThreshIndex += 1
            Next
            If lThreshFound >= 0 Then
                '1st curve is lower limit down to bottom of graph
                vThresh = lStn.Thresholds(lThreshFound)
                If vThresh.LowerLimit < 1.0E+19 OrElse vThresh.UpperLimit < 1.0E+19 Then 'check for both set to 'inf'
                    ...
                    lCurve = lPane.AddCurve(lThreshLegendLabels(lThreshFound), lThrshDates, lThrshVals, lThreshColors(lThreshFound), SymbolType.None)
                    lCurve.Line.Width = 6
                    lCurve.Label.IsVisible = False
                    ...
                    lCurve.Label.IsVisible = False
                End If
            ElseIf lThreshFound < 0 AndAlso lQ.Year >= lStn.Thresholds(0).SYear AndAlso lQ.Year < lStn.Thresholds(0).EYear Then
        ...
        lThrshVals(0) = lYAxis.Scale.Max
        lThrshVals(1) = lYAxis.Scale.Max
    ...
    Public Sub GenFrequencyGraph(ByVal aStnInd As Integer, Optional ByVal aToFile As Boolean = False)
        ...
        For i = lNPlot1 To lNPlot2
            ...
            lXVals(j) = lTxProb(i)
        Next
        If lXVals(0) > lPane.XAxis.Scale.Max Then lPane.XAxis.Scale.Max = lXVals(0)
